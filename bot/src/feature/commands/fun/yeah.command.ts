...
@injectable()
export class YeahCommand implements ICommand {
    name: string = 'yea';
    description: string = 'hey little man hows it goin? Reply to a message to ask them.';
    usageHint: string = '';
    examples: string[] = [''];
    ...
    private async drawImage(user: User, text: string): Promise<AttachmentBuilder> {
        ...
        const background = await loadImage('https://i.kym-cdn.com/photos/images/newsfeed/001/914/391/342.jpg');
